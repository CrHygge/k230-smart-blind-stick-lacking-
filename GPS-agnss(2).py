...
ak =  "qnDHvcEsWy3VsnNTM4W0Nz6VjrUr4jE2"	
baiduUrl = "http://api.map.baidu.com/location/ip?ak=%s&coor=bd09ll" % (ak)
req = requests.get(baiduUrl)
content = req.text
baiduAddr = json.loads(content)

if baiduAddr.get("status") == 0:
    content = baiduAddr.get("content", {})
    city = content.get("address_detail", {}).get("city")
    maplng = content.get("point", {}).get("x")
    maplat = content.get("point", {}).get("y")
else:
    print(baiduAddr.get("message"))
    raise SystemExit
                                ...
                                if ser.inWaiting():
                                    if ser.read(1) == b'G':
                                        if ser.inWaiting():
                                            if ser.read(1) == b'A':
                                                #utctime = ser.read(7)
                                                GGA = ser.readline().decode(errors="ignore")
                                                print(GGA)
